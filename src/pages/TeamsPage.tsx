...
import { Shield, Plus, Pencil, Trash2, Copy, Search, FolderPlus, Folder, FolderOpen, ChevronRight, X, GripVertical } from "lucide-react";
    ...
    >
      ...
      <div className="flex items-center gap-2 pl-2">
        ...
        <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button onClick={onDuplicate} className="p-1.5 rounded-md hover:bg-secondary text-muted-foreground hover:text-foreground">
...
function FolderNode({
  folder, allFolders, teams, openFolders, dragOverFolder,
  editingFolderId, editingFolderName, onToggle, onEdit, onRename,
  onCancelEdit, onEditNameChange, onDeleteFolder, onDragOver,
  onDragLeave, onDrop, onFolderDragStart, navigate, onDuplicate,
  onDeleteTeam, depth = 0,
}: FolderNodeProps) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-0.5" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => onEdit(folder.id, folder.name)} className="p-1 rounded hover:bg-secondary text-muted-foreground hover:text-foreground">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>Os times não serão excluídos, apenas removidos da pasta.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancelar</AlertDialogCancel>
                <AlertDialogAction onClick={() => onDeleteFolder(folder.id, folder.name)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                  Excluir
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
...
export default function TeamsPage() {
  const { teams, removeTeam, addTeam, loading, folders, addFolder, renameFolder, removeFolder, moveTeamToFolder, moveFolderToFolder } = useTournamentStore();
  ...
  const handleDrop = useCallback((e: DragEvent, folderId: string) => {
    e.preventDefault();
    e.stopPropagation();
    setDragOverFolder(null);
        ...
        <div className="flex items-center gap-2">
          {teams.length > 0 && (
            <div className="relative w-full sm:w-56">
              ...
              <Input placeholder="Buscar time..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-9" />
            </div>
          )}
          <button
        ...
        >
          ...
          {rootFolders.map((folder) => (
            <FolderNode
              ...
              onEdit={(id, name) => { setEditingFolderId(id); setEditingFolderName(name); }}
